...
# ──────────────────────────────────────────────────────────────────────────────
def _parse_one(args: tuple) -> tuple[str, str, list, str | None, float]:
    """
    单文件解析 & 分块，运行在独立子进程中。
...
def build_vector_db(data_dir: str = settings.tools.vector_store.data_dir):
    ...
    # ── T1+T2 计时汇总：Pool 结束后打印两个指标 ──────────────────────────
    #
    # 为什么不用 timer()？
    #   timer 是 with 块内的耗时计量器。流水线模式下 Pool 块内同时跑着
    #   T3/T4（_flush_batch），T1+T2 的 wall 时间会把 T3/T4 也包进去，
    #   语义失真。更重要的是：子进程里无法直接调用主进程的 timer，
    #   只能靠 _parse_one 把 elapsed 带回来再在主进程汇总。
    #   结论：T1+T2 直接 print，不进 timer 汇总表；T3/T4 照常进汇总表。
    #
    # wall_elapsed  ：流水线整体挂钟时间（含 T3/T4 重叠部分），
    #                 体现用户实际等待缩短了多少。
    # cpu_parse_total：各子进程 T1+T2 纯耗时之和，
    #                  ≈ 旧版串行 T1+T2 总时间，两者直接可比。
    # parallel_ratio ：cpu_total / wall，越大说明并行效率越高。
